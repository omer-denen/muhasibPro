...
| 288 | 2026-09-18 | **AI asistan devri (diğer model → Muse Spark, imza) + D1 model ölçümleri:** `qwen3-1.7b` ❌ (`<think>` sızıntısı + tekrar döngüsü) · `qwen2.5-1.5b` ❌ (Türkçe halüsinasyon: 1120×2=660) · ders: üretici ≠ proje bilgisi (bilgi RAG'den) · donanım kapısı D1-D6 planı · yarın: crash fix → phi-3.5-mini | 📋 devir+ölçüm (kod yok) | C14 |
