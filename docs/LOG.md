...
## Oturum İndeksi (288 numaralı oturuma kadar, 14 cilt)
...
| 285 | 2026-09-17 | Faz 6.85 **K1** (RBAC temeli) **+ K2** (Kullanıcı Yönetimi modalı) — canlıda FK bug'ı → `IUserRepository` Scoped fix (Kural 8); build 0 + **655/655**; Kural 18 `ot285_s_kayit`. **DEVİR (yeni context):** K2 view'i **reddedildi** → standart `FirmaList`/`FirmaDetails` desenine redesign + **K3 izin matrisi aynı sayfada** + **avatar**; K3 motor iskeleti (`RolYetkiService`) kodlandı (DI/UI/test yok) | ✅🧪 / 🔨 devir | C14 |
| 286 | 2026-09-17 | Firma modülü yeni tasarım (K2-REDESIGN ön koşulu): `Views/Firmalar/*` Kural 17 + `MuhasibCardStyle` (liste Kural-17 kart + detay kartı + Pivot) + FirmaShell "Firma Yönetimi" butonu; `Views/Firma/*` korundu. Karar: ham InfoBar yerine oto-kapanan kanal; yeni sayfada `?` yok. Build 0 + **655/655** + Kural 18 `ot286_*` | ✅🧪 (onay bekliyor) | C14 |
| 287 | 2026-09-18 | K3 izin UI revizyonu (modül listesi + aksiyon kartı + toplu ata + yetki özeti) + ComboBox beyazlaşma fix (legacy şablon → WinUI3 varsayılan). Build 0 + **665/665**. Kapanış: **Kullanıcı Yönetimi açılışında çökme (AÇIK, HATALAR)** — yarın ilk iş | ✅ kod / ⚠️ çökme AÇIK | C14 |
| 288 | 2026-09-18 | **AI asistan (devir + D1 model ölçümleri) — sonradan tek sahipte (Oturum 293):** `qwen3-1.7b` ❌ (`<think>` sızıntısı + tekrar döngüsü) · `qwen2.5-1.5b` ❌ (Türkçe halüsinasyon: 1120×2=660) · ders: üretici ≠ proje bilgisi (bilgi RAG'den) · donanım kapısı D1-D6 planı · yarın: crash fix → phi-3.5-mini | 📋 devir+ölçüm (kod yok) | C14 |
| 289 | 2026-09-18 | **Max 1.5B kısıtı (kullanıcı: fan/CPU/RAM yükü) + ≤1.5B işlem planı:** phi indirmesi durduruldu, 2648 MB önbellek silindi · adaylar `qwen3-0.6b` (593 MB) + `qwen3.5-0.8b` (1037 MB), baz `qwen2.5-0.5b` · plan P1 ölçüm → P2 karar → P3 strip (koşullu) → P4 D2-D6 → P5 6.94 H2 · **ana model incelemesine** (onaysız indirme yok) · kapsam kilidi: yalnız AI asistan | 📋 plan (kod yok) | C14 |
| 290 | 2026-09-18 | **Sabit model + fine-tune kararı:** TAM KİLİT (tek alias ürün sabiti, kullanıcı değiştiremez) · fine-tune RAFTA (Foundry duvarı + ≤1.5B ezber zayıflığı + yaşayan içerik + maliyet) · stabilite planı S1 kilit → S2 temperature ~0 → S3 öğretme=RAG → S4 6-soru regresyon kapısı · kod bulgusu: prompt sadakat kuralı var, temperature/seed yok, alias değişebilir | 📋 karar+plan (kod yok) | C14 |
| 291 | 2026-09-18 | **P1+P2 (ana model emri):** `qwen3-0.6b` ❌ (`<think>` + tekrar döngüsü + RAG-bağlamı yoksayma; 511 MB/+769 MB) · `qwen3.5-0.8b` ❌ (tekrar döngüleri + **Q6 bağlam inkârı**; 1038 MB/+1280 MB) → **kazanan yok, varsayılan `qwen2.5-0.5b` kalır** · P3 düştü (koşul yok) · ~1.5 GB önbellek silindi · sırada P4/S1/S2 (Kural 8 onayı) | ✅ ölçüm+karar (kod yok) | C14 |
| 291 | 2026-09-18 | **Kullanıcı Yönetimi çökme fix (`FormComboBox.UpdateVisualState` NRE — şablon parçası null) + "Roller & İzinler" yeniden tasarım (onaylı):** konteynerli modül listesi + seçili modül aksiyon kartı + `x/y` hapları + **Hazır ayar presetleri** + **değişiklik göstergesi**; yayılı özet kaldırıldı; `ItemTemplate` UIA adı fix. Araştırma `REFERANSLAR` 291. Build 0 + **665/665** + canlı `ot294_*`/`ot296_*`. Kararlar: **6.92 Adım 7 ✅** · **K4 (menüde gizle / butonda pasif+gerekçe)** · **sahiplik katman ayrımı** (yardımcı motor/veri/model; ana model UI/VM/DI/RBAC/içerik/docs). **K4 başladı:** firma-bağımsız `IPermissionService.KullaniciYetkisiVarMiAsync` + **`MainShell` navbar gerçek** (Kural 21: sahte modüller kaldırıldı; yetkisiz menü öğesi gizli) + Denetim bölüm kapıları + FirmaShell "Firma Yönetimi" kapısı + **yıkıcı guard (kısmi: Firma sil/Kullanıcı sil, servis katmanı)**; build 0 + **673/673** + canlı `ot297_*` | ✅🧪 (K3 onaylı) / 🔨 K4 (kalan: DB/log guard + lisans kapısı) | C14 |

| 293 | 2026-09-18 | **S1 UI (ana model): Denetim model seçici kaldırıldı** — `YapayZekaAyarPaneli` "Model" bölümü (TextBox+`Uygula`+`Varsayılan`+ilerleme) silindi, salt-okunur "Model" kartı eklendi; `YapayZekaAyarlarViewModel` `ModelAlias` salt-okunur + `Uygula`/`Varsayılan`/bayraklar/`IsYonetici`/`_auth` silindi (Kural 4); yardım maddeleri güncellendi. Servis zaten sabitti (S2 + S1-servis + D2/D3; yardımcı 292). Build 0 + **683/683** (2 kırmızı test kapandı). Not: 28 test nullable uyarısı HEAD'de de mevcut. | S1 UI (commit bekliyor) | C14 |
---
| 293 | 2026-09-18 | **6.94 H1-H5 tamam + Kural 21 temizligi + yardimci-model donemi kapatildi (tek sahip) + Faz 6.97 + H4 A1:** ? butonlari/altyapi silindi, F1 -> asistan (canli ot298), prompt sertlestirme + ilgi esigi. Build 0 + 687/687 | ✅ (H4/6.97 devam) | C14 |
