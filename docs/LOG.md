...
| 285 | 2026-09-17 | Faz 6.85 **K1** (RBAC temeli: `RolPermission` seed + idempotent migration + KFR + backfill + Yönetici bypass) **+ K2** (Kullanıcı Yönetimi modalı: `UserInfoControl` admin girişi + liste/rol + yeni/düzenle/aktif/şifre/sil) — canlıda FK bug'ı → `IUserRepository` Scoped fix (Kural 8); build 0 + **655/655**; Kural 18 `ot285_s_kayit` | ✅🧪 | C14 |
