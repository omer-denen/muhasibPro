...
| 6.85 | Kullanıcı Yönetimi + RBAC (kullanıcı → modül/alan erişimi) | **K1 ✅🧪 (Oturum 285):** RBAC motoru canlı — `RolPermission` seed (Yönetici 76/Kullanıcı 16) + firma oluşturana Yönetici KFR + açılış backfill + `PermissionService` Yönetici bypass; migration `RbacRolPermissionSeed` (idempotent) dev DB kopyasında doğrulandı; build 0 + 646/646. **K2-K6 📋** (`docs/KULLANICI-YONETIMI-PLAN.md`): K2 kullanıcı yönetimi modal → K3 izin matrisi → K4 modül/alan kapısı → K5 Hesabım/menü → K6 doğrulama. Karar: firma erişimi KFR (oluşturan=sahip=Yönetici), iki rol |
