...
| 6.85 | Kullanıcı Yönetimi + RBAC (kullanıcı → modül/alan erişimi) | 📋 **Plan ✅ + ÖNCELİKLİ FAZ** (Oturum 284, genişletildi): `docs/KULLANICI-YONETIMI-PLAN.md` K1-K6. Kod doğrulaması: RBAC fiilen çalışmıyor (KFR yazılmıyor, `RolPermission` seed yok, UI kapısı yok). **Kararlar:** firma erişimi KFR (oluşturan=sahip=Yönetici), mevcut firmalar önemsiz, **iki rol** (Yönetici/Kullanıcı). Tek kalan: **K1 migration = Kural 8 onayı** |
