...
5. 🔨 **6.94 — Tek Yardım Yüzeyi (AI Asistanı):** plan ✅ (`docs/YARDIM-TEK-YUZEY-PLAN.md`, H1-H5). **H2 içerik ✅ TAM (Oturum 293):** gerçek UI envanteri + 9 mevcut sayfa derinleştirildi/gerçeğe çekildi + **5 yeni sayfa** → `docs/yardim` **14 sayfa / 119 madde** (60 → 119). **Onay bekliyor.** **Kural 21 temizliği ✅ (onaylı):** MainShell Snapshot Yedek + statik tenant/placeholder + Login "Şifremi Unuttum?" kaldırıldı. Sonra H1 (AGENTS Kural 13 revizyonu) → H3 (UI `?` kaldırma + F1) → H5. H4 motor yardımcı modelde.
