...
5. 🔨 **6.94 — Tek Yardım Yüzeyi (AI Asistanı):** plan ✅ (`docs/YARDIM-TEK-YUZEY-PLAN.md`, H1-H5). **H2 içerik (Oturum 293 başladı):** gerçek UI envanteri + **5 yeni sayfa** (`10-denetim-masasi`, `11-sistem-veritabani-yonetimi`, `12-guncelleme-sonrasi`, `13-diyaloglar`, `14-acilis-kurulum`); `docs/yardim` **14 sayfa**. **Kalan H2:** mevcut 9 sayfayı buton/işlem düzeyinde derinleştirme + kullanıcı gözden geçirmesi. Sonra H1 (AGENTS Kural 13 revizyonu) → H3 (UI `?` kaldırma + F1) → H5. H4 motor yardımcı modelde.
...
- **6.94 H2 içerik (başladı):** gerçek UI envanteri (explore, kaynak-doğrulanmış) + **5 yeni yardım sayfası** (`10-denetim-masasi`, `11-sistem-veritabani-yonetimi`, `12-guncelleme-sonrasi`, `13-diyaloglar`, `14-acilis-kurulum`) → `docs/yardim` **9 → 14 sayfa**. Build 0 + **685/685** (içerik testleri mock'lu; sayıya bağlı değil). **Kalan H2:** mevcut 9 sayfayı buton/işlem düzeyinde derinleştirme + kullanıcı gözden geçirmesi. Sonra H1 → H3 → H5; D2/D3 DI + D4/D5 UI (ana model). H4 motor yardımcı modelde.
