...
5. 🔨 **6.94 — Tek Yardım Yüzeyi (AI Asistanı):** plan ✅ (`docs/YARDIM-TEK-YUZEY-PLAN.md`, H1-H5). **H2 içerik ✅ TAM (Oturum 293):** gerçek UI envanteri + 9 mevcut sayfa derinleştirildi/gerçeğe çekildi + **5 yeni sayfa** (`10-denetim-masasi`, `11-sistem-veritabani-yonetimi`, `12-guncelleme-sonrasi`, `13-diyaloglar`, `14-acilis-kurulum`) → `docs/yardim` **14 sayfa / 119 madde** (60 → 119). **Onay bekliyor.** Sonra H1 (AGENTS Kural 13 revizyonu) → H3 (UI `?` kaldırma + F1) → H5. H4 motor yardımcı modelde. **Yan bulgu (Kural 21, ayrı iş):** MainShell "Snapshot Yedek" (Command yok) + statik tenant/ContentFrame yer tutucu + Login "Şifremi Unuttum?" (işlevsiz).
