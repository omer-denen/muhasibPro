...
1. **6.91 kararları ✅ (Oturum 274, onaylandı):** (a) veri yolu → `%AppData%\MuhasibPro`; (b) dönem taraması → açılışta splash adımı (bloklayıcı); (c) ön-yedek → Sistem.db + göç gerektiren dönemler; (d) imza yok → bütünlük (checksum) ile başla, imza sonraki tur; (e) TenantDatabaseUpdateView → **SİL** + erişimde onay/inline göç. Uygulama sırası: 6.91-A veri yolu+release → B ön-yedek+hook → C ileri-uyumluluk guard → D post-update saga → E view silme/refactor → F UX+yardım. **Uygulama başlamadı (kod yok).**
