...
- **NOT (yeni karar bekliyor):** Durum çubuğu yalnız `ShellView` içinde; ana pencere akışı `ShellView` kullanmıyor (`SplashNavigator`→`LoginView`; `LoginViewModel.EnterApplication`→doğrudan `FirmaShellView`). Bu yüzden yeni footer **ana pencerede görünmez**, yalnız DetailsWindow'larda görünür (mevcut mimari; Chunk-1'den önce de böyleydi). "Ana pencereye de bağla (ShellView) / DetailsWindow-only kalsın" kararı kullanıcıya bırakıldı (Oturum 271: "şimdilik not düş, sonra karar").
