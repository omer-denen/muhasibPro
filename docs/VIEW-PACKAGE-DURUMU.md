...
| 05-MainShell | ✅ Eklendi 🧪 | NavSidebarControl 240 + yüzen başlık; **Oturum 253:** Kural 17 Katman 2 — sağ içerik (başlık + tenant kartı + içerik) `AnaBorder` paneline alındı (sidebar zeminde), `?` panel köşesinde, `InventFrostPanelStyle` silindi; canlı Dark/Light (`ot253f`/`ot253i_mainshell_light`) |
...
- **Oturum 253 (Faz 6.80 Katman 2 — güncel yön):** Tüm çekirdek view'lar zemin → ana border → kartlar yapısında (MaliDonemYonetim/TenantDatabaseUpdate/Update/SistemDbYonetim/DatabaseSettings/MainShell); FirmaShell/KurulumSplash hairline + `Translation Z 32`; 10 dialog kökü opak; `?` panel köşesi 5 VM'de; `dark.jpg` silindi, `light.jpg` 2560px. **Görsel dil mimarisi `AGENTS.md` + `TASARIM-KURALLARI.md` (Kural 17); WebToXaml/viewpackage artık referans değil.**
