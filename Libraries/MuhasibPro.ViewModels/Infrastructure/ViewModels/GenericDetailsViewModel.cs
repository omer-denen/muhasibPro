...
public abstract class GenericDetailsViewModel<TModel> : ViewModelBase where TModel : ObservableObject, new()
{
    ...
    virtual public async Task SaveAsync()
    {
        ...
        if (await SaveItemAsync(EditableItem))
        {
            ...
            if (isNew)
            {
                MessageService.Send(this, EntityEvents.NewItemSaved, Item);
                NotificationService.Show("Başarılı", "Yeni kayıt başarıyla oluşturuldu.", NotificationType.Success);
            }
            else
            {
                MessageService.Send(this, EntityEvents.ItemChanged, Item);
                NotificationService.Show("Başarılı", "Değişiklikler başarıyla kaydedildi.", NotificationType.Success);
            }
        ...
        await ExecuteActionAsync(
            action: async () =>
            {
                if(model != null)
                {
                    ...
                    {
                        MessageService.Send(this, EntityEvents.ItemDeleted, model);
                        await NavigationService.CloseViewAsync();
                    }
                }
