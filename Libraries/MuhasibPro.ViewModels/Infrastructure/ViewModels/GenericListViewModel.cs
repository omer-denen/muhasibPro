    ...
    public TModel SelectedItem
    {
        ...
        set
        {
            if(Set(ref _selectedItem, value))
            {
                ...
                {
                    // Görsel seçim container VSM'ye değil model.Selected'a bağlı —
                    // tek modelin Selected=true olmasını burada garanti et (List<Liste> desync güvenli)
                    SyncSelectedFlags(_selectedItem);
                    MessageService.Send(this, EntityEvents.ItemSelected, _selectedItem);
                }
            }
        }
    }

    private void SyncSelectedFlags(TModel selected)
    {
        if(ItemsSource == null)
            return;
        foreach(var item in ItemsSource)
        {
            bool shouldSelect = ReferenceEquals(item, selected);
