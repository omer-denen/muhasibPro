...
public class SistemDbYonetimViewModel : ViewModelBase
{
    ...
    private readonly SistemYedekViewModel _yedek;
    private readonly SistemGuncellemeViewModel _guncelleme;

    public SistemDbYonetimViewModel(
        ICommonServices commonServices,
        IApplicationPaths appPaths,
        ISistemDatabaseService sistemDatabaseService,
        ISistemDatabaseOperationService operasyonService,
        IDatabaseSettingsProvider ayarSaglayici,
        ...
        ISistemRestoreAnalizService restoreAnalizService) : base(commonServices)
    {
        ...
        _yedek.PropertyChanged += (_, e) => { if (e.PropertyName == nameof(IsBusy)) NotifyPropertyChanged(nameof(IsBusy)); };
        _guncelleme.PropertyChanged += (_, e) => { if (e.PropertyName == nameof(IsBusy)) NotifyPropertyChanged(nameof(IsBusy)); };
    ...
    // Splash yönlendirmesiyle gelindiğinde güncelleme durumunu netleştir (karar izi günlüğe düşer — Adım 0 tanısı)
    public void SetMigrationMode(string? kararOzeti = null)
    {
        if (!string.IsNullOrWhiteSpace(kararOzeti))
            AddLog($"[YÖNLENDİRME] Splash bu sayfaya yönlendirdi: {kararOzeti}");
        UpdateStatusMessage();
    ...
    public SistemYedekViewModel Yedek => _yedek;
    public SistemGuncellemeViewModel Guncelleme => _guncelleme;
    ...
    {
        get => base.IsBusy || _status.IsBusy || _creation.IsBusy || _diagnostics.IsBusy || _kurulum.IsBusy || _yedek.IsBusy || _guncelleme.IsBusy;
    ...
    private async Task LoadInitialStateAsync()
    {
        ...
        try { await _yedek.YukleAsync(); } catch (Exception ex) { AddLog($"[HATA] Yedek listesi yüklenemedi: {ex.Message}"); }
        try { await _guncelleme.YukleAsync(); } catch (Exception ex) { AddLog($"[HATA] Güncelleme listesi yüklenemedi: {ex.Message}"); }
        UpdateStatusMessage();
        NotifyAll();
    }
    ...
    private void UpdateStatusMessage()
    {
        ...
        if (DbExists && IsDbValid)
            ...
            StatusMessage = "Güncelleme bekleniyor";
    }
