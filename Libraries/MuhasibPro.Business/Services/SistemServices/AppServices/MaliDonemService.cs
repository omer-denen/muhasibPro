    ...
    public class MaliDonemService : IMaliDonemService
    {
        ...
        public async Task<ApiDataResponse<MaliDonemModel>> CreateNewMaliDonemForFirmaAsync(long firmaId)
        {
            try
            {
                ...
                // Yeni dönem varsayılan durumu EntityRegistrySettings'ten (kart/seed bu değeri devralır).
                var entityAyar = _entityAyarlar != null ? await _entityAyarlar.GetAsync() : new EntityRegistrySettings();
                model.TenantDetails = new TenantDetailsModel { Durum = entityAyar.DefaultDurum };
