...
namespace MuhasibPro.Business.Services.DatabaseServices.TenantDatabaseService
{
    public class TenantSQLiteDatabaseOperationService : ITenantSQLiteDatabaseOperationService
    {
            ...
            try
            {
                ...
                if (result.IsBackupComleted)
                {
                  ...
                  $"Yedeklenen Veritabanı dosyası : {result.BackupFileName} Boyut: {result.BackupFileSizeBytes}");
                    ...
                    _eventBus?.Publish(this, new TenantBackupCompletedEvent(databaseName, result.BackupFileName ?? string.Empty));
        ...
        public async Task<ApiDataResponse<List<DatabaseBackupResult>>> GetAllBackupsAsync()
        {
            try
            {
                var backups = await _backupManager.GetAllBackupsAsync();
                return new SuccessApiDataResponse<List<DatabaseBackupResult>>(
                    data: backups,
                    message: $"{backups.Count} yedek bulundu.",
                    resultCount: backups.Count);
            }
            catch (Exception ex)
            {
                await _logService.SistemLogExceptionAsync("Mali Dönem Veritabanı Yedek Listesi", "Tüm Yedekler", ex);
                return new ErrorApiDataResponse<List<DatabaseBackupResult>>(
                    data: null,
        ...
        public async Task<ApiDataResponse<TenantDerinAnaliz>> GetDerinAnalizAsync(string databaseName)
        {
            try
            {
                var analiz = await _databaseManager.GetDerinAnalizAsync(databaseName);
                if (!string.IsNullOrEmpty(analiz.Hata))
                    return new ErrorApiDataResponse<TenantDerinAnaliz>(data: analiz, message: analiz.Hata, resultCount: 0);
                return new SuccessApiDataResponse<TenantDerinAnaliz>(data: analiz, message: "Derin analiz tamamlandı.", resultCount: 1);
            }
            catch (Exception ex)
            {
                await _logService.SistemLogExceptionAsync("Mali Dönem Veritabanı Analiz", "Derin Analiz", ex);
                return new ErrorApiDataResponse<TenantDerinAnaliz>(data: null, message: "Derin analiz sırasında teknik bir hata oluştu.", resultCount: 0);
            }
        }
        public async Task<ApiDataResponse<DatabaseMaintenanceResult>> BakimCalistirAsync(string databaseName, string komut)
        {
            try
            {
                // Bakım zaman aşımı kullanıcı ayarından (yoksa 120 sn varsayılanı manager uygular)
                int? timeoutSec = null;
                try
                {
                    if (_settingsProvider != null)
                        timeoutSec = (await _settingsProvider.GetAsync())?.GetBakimTimeoutSec();
                }
                catch { }
                var sonuc = await _databaseManager.BakimCalistirAsync(databaseName, komut, timeoutSec);
                if (!sonuc.Basarili)
                    return new ErrorApiDataResponse<DatabaseMaintenanceResult>(data: sonuc, message: sonuc.Mesaj, resultCount: 0);
                await _logService.SistemLogInformationAsync("Mali Dönem Veritabanı Bakım", komut, "İşlem başarılı", $"{databaseName}: {sonuc.Mesaj}");
                return new SuccessApiDataResponse<DatabaseMaintenanceResult>(data: sonuc, message: sonuc.Mesaj, resultCount: 1);
            }
            catch (Exception ex)
            {
                await _logService.SistemLogExceptionAsync("Mali Dönem Veritabanı Bakım", komut, ex);
                return new ErrorApiDataResponse<DatabaseMaintenanceResult>(data: null, message: "Bakım sırasında teknik bir hata oluştu.", resultCount: 0);
            }
        }
      ...
      string backupFileName)
        {
            try
            {
                ...
                if (result.IsRestoreSuccess)
                {
                    ...
                    _eventBus?.Publish(this, new TenantRestoreCompletedEvent(databaseName, backupFileName));
