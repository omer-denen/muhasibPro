...
namespace MuhasibPro.Business.Extensions.SistemService.AppService
{
    public static class FirmaServiceExtensions
    {
        ...
        /// <summary>Liste kartı için hafif dönem projeksiyonu (kart sayacı bu listeden beslenir).</summary>
        public static List<MaliDonemModel> ToHafifDonemListesi(IEnumerable<MaliDonem> donemler)
        {
            if (donemler == null)
                return new List<MaliDonemModel>();
            return donemler.Select(md => new MaliDonemModel
            {
                Id = md.Id,
                FirmaId = md.FirmaId,
                MaliYil = md.MaliYil,
        ...
        /// <summary>Firma + dönemleriyle kart modeli (seçim aynası bu projeksiyonu kullanır).</summary>
        public static FirmaModel ToDonemliFirmaModel(Firma f)
        {
            return new FirmaModel
            {
                ...
                MaliDonemler = f.MaliDonemler != null
                    ? f.MaliDonemler
                        .Select(
