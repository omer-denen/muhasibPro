    ...
    public class TenantSQLiteBackupManager : ITenantSQLiteBackupManager
    {
            ...
            try
            {
                ...
                try
                {
                        ...
                        Path.GetFileName(backupFileName)))
                    {
                        ...
                        // Kaynak tenant DB dosya boyutu (kart vitrini DosyaBoyutu için).
                        result.BackupFileSizeBytes = new FileInfo(sourcePath).Length;
                    }
                    else
                    {
        ...
        public Task<List<DatabaseBackupResult>> GetBackupsAsync(string databaseName)
        {
            try
            {
                ...
                // Üretim formatı çıplak addır ({db}_{zaman}_{guid}.backup); okuma tarafı
                // tarihsel olarak dosya adını (.db ile) bekliyordu — iki desen de taranır.
                var desenler = new[]
                {
                    $"{databaseName}_*.backup",
                    $"{Path.GetFileName(GetTenantFilePath(databaseName))}_*.backup"
                };
                ...
                {
                    foreach (var filePath in Directory.GetFiles(backupDir, desen))
                    {
                        try
                        {
                            if (!gorulen.Add(filePath))
                                continue;
                            var fileInfo = new FileInfo(filePath);
                            // Önek çakışması filtresi: çıkarılan ad istenen döneme ait olmalı.
                            if (!DatabaseNameEslesiyor(ExtractDatabaseName(fileInfo.Name), databaseName))
                                continue;
        ...
        /// <summary>
        /// Yedek klasöründeki TÜM tenant yedekleri (öksüz taraması için).
        /// DatabaseName dosya adından çözülür (`db-KODU_YIL.db_rest...` → `db-KODU_YIL.db`).
        /// </summary>
        public Task<List<DatabaseBackupResult>> GetAllBackupsAsync()
        {
            try
            {
                var backupDir = GetTenantBackupFolderPath;
                if (!Directory.Exists(backupDir))
                    return Task.FromResult(new List<DatabaseBackupResult>());
                var backupList = new List<DatabaseBackupResult>();
                foreach (var filePath in Directory.GetFiles(backupDir, "db-*.backup"))
                {
                    try
                    {
                        var fileInfo = new FileInfo(filePath);
                        var isValidBackup = _backupManager.IsValidBackupFile(backupDir, fileInfo.Name);
                        var isSqliteValid = _applicationPaths.IsSqliteDatabaseFileValid(filePath);
                        var br = new DatabaseBackupResult
                        {
                            BackupPath = filePath,
                            BackupDirectory = backupDir,
                            BackupFileName = fileInfo.Name,
                            ...
                            LastBackupDate = fileInfo.LastAccessTimeUtc,
                            BackupType = _backupManager.DetermineBackupType(fileInfo.Name),
                            DatabaseName = ParseDatabaseName(fileInfo.Name),
                            IsBackupComleted = isValidBackup && isSqliteValid,
                        };
                        FillBackupIdentity(br);
                        backupList.Add(br);
                    }
                    catch
                    {
            ...
            {
                _logger?.LogDebug(ex, "Tüm yedek listesi alınamadı");
                return Task.FromResult(new List<DatabaseBackupResult>());
            }
        }

        private static void FillBackupIdentity(DatabaseBackupResult backup)
        {
            try
            {
                if (backup == null || string.IsNullOrWhiteSpace(backup.BackupPath) || !File.Exists(backup.BackupPath))
                    return;
                using var conn = new SqliteConnection($"Data Source={backup.BackupPath};Mode=ReadOnly;");
                conn.Open();
                using var cmd = conn.CreateCommand();
                cmd.CommandText = "SELECT FirmaId, MaliDonemId, OlusturanKullaniciId, MakineId, KurulumId, CurrentTenantDbVersion FROM TenantDatabaseVersiyonlar LIMIT 1;";
                using var reader = cmd.ExecuteReader();
                if (reader.Read())
                {
                    backup.KimlikFirmaId = reader.IsDBNull(0) ? null : reader.GetInt64(0);
                    backup.KimlikMaliDonemId = reader.IsDBNull(1) ? null : reader.GetInt64(1);
                    if (!reader.IsDBNull(2))
                    {
                        var val = reader.GetInt32(2);
                        if (Enum.IsDefined(typeof(MuhasibPro.Domain.Entities.SistemEntity.KullaniciRolTip), val))
                            backup.KimlikRol = (MuhasibPro.Domain.Entities.SistemEntity.KullaniciRolTip)val;
                        else
                            backup.KimlikRol = MuhasibPro.Domain.Entities.SistemEntity.KullaniciRolTip.Yönetici;
                    }
                    backup.KimlikMakineId = reader.IsDBNull(3) ? null : reader.GetString(3);
                    backup.KimlikKurulumId = reader.IsDBNull(4) ? null : reader.GetString(4);
                    backup.KimlikVersion = reader.IsDBNull(5) ? null : reader.GetString(5);
                }
            }
            catch { /* eski yedek veya tablo yok -> kimliksiz */ }
        }

        /// <summary>
        /// Yedek dosya adından dönem adı çıkarır. İki format tanınır:
        /// çıplak ({db}_{zaman}_{guid}.backup) ve dosya-adlı ({db}.db_{zaman}_{guid}.backup).
        /// Dönüş her zaman uzantısız kanonik addır (Global.db MaliDonem.DatabaseName ile aynı).
        /// </summary>
        private static readonly Regex _backupNameRegex = new(
            @"^(?<db>.+)_(?<ts>\d{8}_\d{6})_(?<g>[0-9a-fA-F]{4})(?<suffix>_.*)?\.backup$",
            RegexOptions.Compiled | RegexOptions.CultureInvariant);

        private static string ExtractDatabaseName(string backupFileName)
        {
            var m = _backupNameRegex.Match(backupFileName ?? string.Empty);
            if (!m.Success)
                return string.Empty;
            return TrimDbSuffix(m.Groups["db"].Value);
        }

        private static string TrimDbSuffix(string name)
        {
        ...
        private static bool DatabaseNameEslesiyor(string aday, string beklenen) =>
            string.Equals(TrimDbSuffix(aday), TrimDbSuffix(beklenen), StringComparison.OrdinalIgnoreCase);

        private static string ParseDatabaseName(string backupFileName) => ExtractDatabaseName(backupFileName);
            ...
            string backupFileName)
        {
            ...
            catch (Exception ex)
            {
                ...
                // Hata durumunda safety backup'a geri dön (tenant yolu — Sistem.db asla hedef olamaz)
                try
                {
                    var tenantYolu = _applicationPaths.GetTenantDatabaseFilePath(databaseName);
                    await RollbackToSafetyBackupAsync(tenantYolu, safetyBackupPath);
                }
