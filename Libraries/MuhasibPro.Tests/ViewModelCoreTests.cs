...
{
    [Fact]
    public void SistemKurulumViewModel_EF_Referansi_Icermemeli()
    {
        var vmPath = Path.Combine(GetRepoRoot(), "Libraries/MuhasibPro.ViewModels/ViewModels/Sistem/SistemKurulumViewModel.cs");
    ...
    [Fact]
    public void SistemKurulumViewModel_Composition_Kullanmali_Inheritance_Degil()
    {
        var type = typeof(MuhasibPro.ViewModels.ViewModels.Sistem.SistemKurulumViewModel);
        type.BaseType!.Name.Should().Be("ViewModelBase");
        // Kendi içinde 3 alt VM composition
        var src = File.ReadAllText(Path.Combine(GetRepoRoot(), "Libraries/MuhasibPro.ViewModels/ViewModels/Sistem/SistemKurulumViewModel.cs"));
        ...
        src.Should().NotContain("class SistemKurulumViewModel : FirmalarViewModel");
    }

    [Fact]
    public void ViewModel_Sadece_BusinessContracts_Bilmeli()
    {
        var asm = typeof(MuhasibPro.ViewModels.ViewModels.Sistem.SistemKurulum.SistemDatabaseStatusViewModel).Assembly;
        ...
        var vmPath = Path.Combine(GetRepoRoot(), "Libraries/MuhasibPro.ViewModels/ViewModels/Sistem/SistemKurulum/SistemDatabaseStatusViewModel.cs");
        var src = File.ReadAllText(vmPath);
    ...
    [Fact]
    public void XamlModulerligi_SistemKurulum_300_Satiri_Asmamali()
    {
        var viewPath = Path.Combine(GetRepoRoot(), "MuhasibPro/Views/SistemKurulum/SistemKurulumView.xaml");
