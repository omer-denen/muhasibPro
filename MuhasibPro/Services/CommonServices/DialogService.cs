...
namespace MuhasibPro.Services.CommonServices
{
    public class DialogService : IDialogService
    {
        ...
        public async Task<TenantUpdateDecision> ShowTenantUpdateConfirmAsync(TenantUpdateCheckResult check)
        {
            var dialog = new TenantDatabaseUpdateDialog();
            dialog.SetState(check);
            var result = await DialogHelper.ShowCenteredAsync(dialog);
            return result switch
            {
                ContentDialogResult.Primary => TenantUpdateDecision.SimdiGuncelle,
