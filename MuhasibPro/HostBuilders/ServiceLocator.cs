...
namespace MuhasibPro.HostBuilders
{
    public class ServiceLocator : IDisposable
    {
        ...
        {
            DisposeScope(WindowHelper.GetActiveWindowId());
        }

        /// <summary>Pencere kapanış temizliği — o pencerenin scope'u dispose edilir.
        /// Kapanan pencere artık aktif olmayabileceği için id ile çalışır.</summary>
        public static void DisposeScope(int windowId)
        {
            if (_serviceLocators.TryRemove(windowId, out ServiceLocator locator))
            {
