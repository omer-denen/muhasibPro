...
namespace MuhasibPro.Converters
{
    public class StepDisplayConverter : IValueConverter
    {
        ...
        {
            // Oluşturma pipeline ile aynı açık renkler (SagaStepStatusToBrush ile uyumlu)
            if (value is CreationStep cStep && parameter is string brushKind && (brushKind == "Background" || brushKind == "Border" || brushKind == "IconBg"))
            {
                var st = cStep.Status;
